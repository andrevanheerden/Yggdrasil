const pool = require("../config/db");
const safeValue = (val) => (val === undefined || val === null ? "" : val);

const createEncounterAction = async (data) => {
  const sql = `
    INSERT INTO encounter_actions (
      encounter_action_id, encounter_id, action_name, action_type,
      action_image, action_description, action_range, action_area,
      action_cost, action_effects, damage_types
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;

const values = [
  safeValue(data.encounter_action_id),
  safeValue(data.encounter_id),
  safeValue(data.action_name),
  safeValue(data.action_type),
  safeValue(data.action_image),
  safeValue(data.action_description),
  safeValue(data.action_range),
  safeValue(data.action_area),
  safeValue(data.action_cost),
  JSON.stringify(data.action_effects),
  JSON.stringify(data.damage_types),
];


  const [result] = await pool.execute(sql, values);
  return result;
};


const getActionsByEncounter = async (encounterId) => {
  const [rows] = await pool.execute(
    "SELECT * FROM encounter_actions WHERE encounter_id = ?",
    [encounterId]
  );
  return rows;
};

const getActionById = async (id) => {
  const [rows] = await pool.execute(
    "SELECT * FROM encounter_actions WHERE encounter_action_id = ?",
    [id]
  );
  return rows[0];
};

const deleteEncounterAction = async (id) => {
  const [result] = await pool.execute(
    "DELETE FROM encounter_actions WHERE encounter_action_id = ?",
    [id]
  );
  return result;
};

module.exports = {
  createEncounterAction,
  getActionsByEncounter,
  getActionById,
  deleteEncounterAction,
  safeValue,  // ✅ Add this
};

